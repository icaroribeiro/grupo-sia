...
from typing import Dict, Any, List  # Added Optional
...
class BaseDashboardTab:
    ...
    # ... get_agent_format_instructions is unchanged ...
    ...
    def _make_choropleth(self, df: pd.DataFrame, geojson: dict, color_theme: str):
        ...
        st.markdown(f"🗺️ **Mapa de {label} por UF Emitente**")  # Bolded title
    ...
    # ... _make_bar_chart_by_year_and_uf is unchanged ...
    ...
    # ... _render_summary_metrics is unchanged ...
    ...
    # ... _render_data_table is unchanged ...
    def _render_data_table(self, df_map_data: pd.DataFrame):
        metric = self.METRIC_COLUMN
        label = self.METRIC_LABEL
    ...
    # --- FIX APPLIED HERE: Conditional rendering based on GROUP_BY_COLUMN ---
    def render(
        self,
        selected_year: int,
        ...
        if not df_map_data.empty:
            ...
            if self.GROUP_BY_COLUMN == "uf_code":
                df_map_data["geojson_id"] = df_map_data["uf_code"].apply(_to_geojson_id)

        # --- REWRITTEN LAYOUT: Map/Chart (5) left, Metrics & Table (3) right ---
        ...
        with col_left:
            ...
            if self.GROUP_BY_COLUMN == "uf_code":
                ...
                # 2. Chart below the map (Multi-Year Bar Chart)
                df_bar_chart = df_multi_year.copy()
                # Check for required columns and ensure data is present
                required_cols = ["uf_code", "year", self.METRIC_COLUMN]

                # Check if multi-year data has the required metric for the current tab
                if all(col in df_bar_chart.columns for col in required_cols):
                    ...
                    df_bar_chart = pd.DataFrame()  # Empty if metric not found
                ...
                # Placeholder for non-UF tabs (e.g., Natureza da Operação, Modalidade de Venda)
                # Display only the data table and metrics on the right side.
                st.info(
        ...
        with col_right:
            # 3. Summary Metrics (now 2x2 inside)
            self._render_summary_metrics(df_map_data)

            st.markdown("---")

            # 4. Data Table (extended size by column ratio)
            self._render_data_table(df_map_data)
        # -----------------------------------------------------------------------

        # Display raw data expander (only for UF-grouped tabs, as others are simple lists)
        if not df_map_data.empty and self.GROUP_BY_COLUMN == "uf_code":
            with st.expander(f"Ver Dados Brutos Utilizados ({self.METRIC_LABEL})"):
                ...
                # Ensure we only try to map UF to state name if the column exists
                if "uf_code" in df_display.columns:
                    ...
                    # Fallback for data table display if uf_code is not present unexpectedly
                    df_display = df_display.rename(
                        columns={
                            self.METRIC_COLUMN: self.METRIC_LABEL,
...
# --- Metric Tabs (unchanged, just included for completeness) ---
...
# --- InvoiceAnalysisPage Class ---


class InvoiceAnalysisPage:
    @inject
    def __init__(
        self,
        ...
        self.tabs = {
            ...
            BuyerPresenceTab.TAB_TITLE: BuyerPresenceTab(self),  # NEW TAB ADDED
        }

    # ... _load_brazil_geojson is unchanged ...
    ...
    # ... _run_dummy_data_simulation is unchanged ...
    def _run_dummy_data_simulation(
        self, selected_year: int
    ) -> Dict[str, List[Dict[str, Any]]]:
        # --- Data for the selected year (UF Grouping) ---
        ...
        return {
            ...
            "presence_data": presence_data,  # NEW DATA ADDED
            "multi_year_data": dummy_multi_year_data,
        }

    # ... show method is where the fix logic is applied in BaseDashboardTab.render call ...
    def show(self) -> None:
        st.title("📊 Análise de NF-e")
        ...
        with st.spinner(
            ...
            try:
                # Assuming this function is stable and returns the data structure as expected
                data = self._run_dummy_data_simulation(selected_year)
                df_multi_year = pd.DataFrame(data["multi_year_data"])

            except Exception as e:
                # Error handling for dummy simulation/workflow
        ...
        for tab_title, st_tab in zip(tab_titles, st_tabs):
            with st_tab:
                ...
                else:  # 'buyer_presence' (Modalidade de Venda)
                ...
                # Filter and prepare data specific to the current tab's metric and group-by column
                data_for_map = [
                    {
                ...
                tab_instance.render(
                    ...
                    df_multi_year,  # df_multi_year is only used if GROUP_BY_COLUMN == "uf_code" inside render
                    geojson,
                    data_for_map,
                )
