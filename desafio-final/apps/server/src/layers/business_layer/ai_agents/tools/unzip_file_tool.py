...
class UnzipFileTool(BaseTool):
    ...
    description: str = """
    Extracts files from ZIP archives to a destination directory.
    
    Args:
        dir_path (str): Path to the ZIP archives directory.
        destionation_dir_path (str): Path to the destination directory.
    
    Returns:
        Union[str, list[str] | None]: Status message indicating success, warning or 
        failure along with list of extracted file paths on success or 'None' on failure.
    """

    def _run(
        self, dir_path: str, destionation_dir_path: str
    ) -> Union[str, list[str] | None]:
        logger.info("Started unzipping file...")
        try:
            extracted_file_paths: list[str] = list()
            os.makedirs(destionation_dir_path, exist_ok=True)
            for file_path in Path(dir_path).glob("*.zip"):
                try:
                    ...
                    message = f"Success: ZIP archive {file_path} extracted: {','.join(extracted_file_paths)}"
                    logger.info(message)
            ...
            message = f"Error: Failed to extract files from ZIP archives: {error}"
            logger.eror(message)
            return (message, None)
