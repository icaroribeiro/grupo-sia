...
class AnalyzeDataTask:
    def create(self, user_query: str, available_dfs_str: str, agent: Agent) -> Task:
        return Task(
            description=f"""
                Analyze the available dataframes ({available_dfs_str}) to answer the following user query: '{user_query}'.
            ...
            """,
            expected_output="""
                A concise summary of the data analysis findings, addressing the user query.
            """,
            agent=agent,
        )
