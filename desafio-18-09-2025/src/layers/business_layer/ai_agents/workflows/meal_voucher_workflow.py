...
class MealVoucherWorkflow(BaseWorkflow):
    ...
    def supervisor_node(
        ...
        return create_react_agent(
            ...
            prompt=(
                ...
                - You're a supervisor.
                GOAL:
                - Your sole purpose is to manage three nodes:
                A data wrangling node. Assign data wrangling-related tasks to this node.
                A data analysis node. Assign data analysis-related tasks to this node.
                A data reporting node. Assign data reporting-related tasks to this node.
                INSTRUCTIONS:
                ...
                - ALWAYS assign work to one node at time.
                - DO NOT call nodes in parallel.
                """
            ),
            name="supervisor",
        )

    def data_wrangling_node(
        self,
        state: StateGraph,
        dataframes_dict: dict[str, DataFrameParams],
    ) -> StateGraph:
        ...
        formatted_custom_prefix: str = self.__create_dataframe_description(
            sorted_items=sorted_items, dataframes_dict=dataframes_dict
        )
        input: str = """
            INSTRUCTIONS:
        ...
        agent: AgentExecutor = create_pandas_dataframe_agent(
            ...
            suffix=self.__AGENT_CUSTOM_SUFFIX,
            verbose=True,
        )
        ...
        dataframes_dict["df_partial_wrangling_result_1"] = DataFrameParams(
            name="df_partial_wrangling_result_1", content=df
        )
        sorted_items = sorted(
            {
                1: "df_partial_wrangling_result_1",
        ...
        input: str = """
            ...
            1. From the `df1` (df_partial_wrangling_result_1) and `df2` (df_apprentice_employee) DataFrames:
            - Perform LEFT merge operation with `df1` and `df2` DataFrames on `employee_id` column;
            - If there are rows in the `df2` DataFrame not included in `df1` DataFrame based on `employee_id` column, concatenate them;
        ...
        dataframes_dict["df_partial_wrangling_result_2"] = DataFrameParams(
            name="df_partial_wrangling_result_2", content=df
        )
        sorted_items = sorted(
            {
                1: "df_partial_wrangling_result_2",
        ...
        input: str = """
            ...
            1. From the `df1` (df_partial_wrangling_result_2) and `df2` (df_employee_abroad) DataFrames:
        ...
        dataframes_dict["df_partial_wrangling_result_3"] = DataFrameParams(
            name="df_partial_wrangling_result_3", content=df
        )
        sorted_items = sorted(
            {
                1: "df_syndicate_working_days",
                2: "df_syndicate_meal_voucher_value",
                3: "df_partial_wrangling_result_3",
            }.items()
        )
        formatted_custom_prefix: str = self.__create_dataframe_description(
            sorted_items=sorted_items,
        ...
        input: str = """
            ...
            2. From the `df3` (df_partial_wrangling_result_3) and `resulting` DataFrames:
        ...
        def extract_absense_return_date_helper(detail: str) -> str | None:
            tool_message = self.extract_absense_return_date_tool._run(
                detail=detail, tool_call_id=str(uuid.uuid4())
            )
            pattern = r"return_date:(.+)"
            match = re.search(pattern, tool_message.content)
            if match:
        ...
        dataframes_dict["df_partial_wrangling_result_4"] = DataFrameParams(
            name="df_partial_wrangling_result_4", content=df
        )
        sorted_items = sorted(
            {
                1: "df_partial_wrangling_result_4",
        ...
        input: str = """
            ...
            1. From the `df1` (df_partial_wrangling_result_4) and `df2` (df_employee_absense) DataFrames:
            - Perform LEFT merge operation with `df1` and `df2` DataFrames on `employee_id` column.
            - If there are rows in the `df2` DataFrame not included in `df1` DataFrame based on `employee_id` column, concatenate them.
        ...
        dataframes_dict["df_partial_wrangling_result_5"] = DataFrameParams(
            name="df_partial_wrangling_result_5", content=df
        )
        ...
        def absense_days_helper(row) -> str | None:
            ...
            tool_message = self.calculate_absense_days_tool._run(
                date_str=date_str,
                working_days_by_syndicate_name=working_days_by_syndicate_name,
                syndicate_name=syndicate_name,
                tool_call_id=str(uuid.uuid4()),
            )
            pattern = r"absense_days:(.+)"
            match = re.search(pattern, tool_message.content)
            if match:
                return match.group(1)
            return None

        df_partial_wrangling_result_5 = dataframes_dict[
            "df_partial_wrangling_result_5"
        ].content
        df_partial_wrangling_result_5["absense_days"] = (
            df_partial_wrangling_result_5.apply(absense_days_helper, axis=1)
        )
        dataframes_dict[
            "df_partial_wrangling_result_5"
        ].content = df_partial_wrangling_result_5
        sorted_items = sorted(
            {
                1: "df_partial_wrangling_result_5",
            }.items()
        )
        formatted_custom_prefix: str = self.__create_dataframe_description(
            sorted_items=sorted_items,
        ...
        input: str = """
            ...
            1. Save the `df1` (df_partial_wrangling_result_5) DataFrame to a CSV file to the path `{output_path}`.
            """
        formatted_input: str = input.format(
            output_path=os.path.join(
                f"{app_settings.output_data_dir_path}",
    ...
    def data_analysis_node(
        ...
        dataframes_dict["df_partial_wrangling_result_6"] = DataFrameParams(
            name="df_partial_wrangling_result_6", content=df
        )
        sorted_items = sorted(
            {
                1: "df_partial_wrangling_result_6",
                2: "df_employee_absense",
        ...
        input: str = """
            ...
            1. From the `df1` (df_partial_wrangling_result_6) DataFrame:
            ...
            2. From the `resulting` and `df2` (df_employee_absense) DataFrames:
            - Filter out rows in `resulting` DataFrame where the value in `employee_id` column is the same value in `employee_id` column from `df2` DataFrame only if `return_date` column is missing or is not a date in May month.
        ...
        input: str = """
            ...
            - Add a new column named `effective_working_days` and fill out the rows with the value of Formula: 
                ...
                - For each row, if the value of `vacation_days` is equal to 30, assign the value 0 to `effective_working_days`.
                - For each row, if the value of `effective_working_days` is greater than `working_days`, assign the value of `working_days` to `effective_working_days`.
        ...
        all_results.append(result)
        # Extract the 'output' key from each result dictionary and join them with a separator.
        final_content = "\n\n---\nData Analysis Step Finished\n---\n\n".join(
            [str(res.get("output", "No output returned.")) for res in all_results]
        )
        return {"messages": [HumanMessage(content=final_content)]}

    def data_reporting_node(
        self,
        state: StateGraph,
        dataframes_dict: dict[str, DataFrameParams],
    ) -> StateGraph:
        ...
        input: str = """
            ...
            - Add a new column named `OBS GERAL` and leave it empty.
    ...
    def __create_dataframe_description(
        ...
        return self.__AGENT_CUSTOM_PREFIX.format(
    ...
    def __build_graph(self) -> StateGraph:
        ...
        builder.add_node(
            node="supervisor",
            action=functools.partial(self.supervisor_node),
            destinations={
                "data_wrangling_node": "data_wrangling_node",
                "data_analysis_node": "data_analysis_node",
                "data_reporting_node": "data_reporting_node",
            },
        )
        builder.add_node(
            node="data_wrangling_node",
            action=functools.partial(
                self.data_wrangling_node, dataframes_dict=self.dataframes_dict
            ),
        )
        builder.add_node(
            node="data_analysis_node",
            action=functools.partial(
                self.data_analysis_node, dataframes_dict=self.dataframes_dict
            ),
        )
        builder.add_node(
            node="data_reporting_node",
            action=functools.partial(
                self.data_reporting_node, dataframes_dict=self.dataframes_dict
            ),
        )

        builder.add_edge(START, "supervisor")
        builder.add_edge("data_wrangling_node", "supervisor")
        builder.add_edge("data_analysis_node", "supervisor")
        builder.add_edge("data_reporting_node", "supervisor")
