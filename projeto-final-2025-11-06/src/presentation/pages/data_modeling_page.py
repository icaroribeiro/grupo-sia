...
class DataModelingPage:
    ...
    def show(self) -> None:
        ...
        st.write(
            ...
            A tabela `invoice_items` está ligada à tabela `invoices` através da **`access_key` (Chave de Acesso)**, 
            e não pelo ID primário UUID. Esta é uma relação de **um-para-muitos (1:N)**.
            """
        )

        st.markdown("#### 1. Tabela `invoices` (Cabeçalho da Fatura)")
        st.markdown(
        ...
        st.markdown(
            "Esta tabela é desnormalizada e armazena os detalhes dos produtos e serviços, ligada à `invoices` pela `access_key`. **Chave Primária:** (`id`)."
        )
        st.info(
        ...
        st.markdown(
            ...
            * **`invoices`**: Chave Primária Composta por **`id`** e **`access_key`**.
            * **`invoice_items`**: Chave Primária **`id`**.
            
            #### Chave Estrangeira (`FK` - Foreign Key)
            * **`invoice_items.access_key`** referencia **`invoices.access_key`**.
            
            #### Restrição de Unicidade
            * **`invoices`**: **`access_key`** é única na tabela de faturas.
            * **`invoice_items`**: Existe uma restrição de unicidade composta por **(`access_key`, `product_number`)** para garantir que a combinação de fatura e item seja única.
            """
        )
