...
# Log the attempt to migrate the PostgreSQL database
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Migrating PostgreSQL database..."

# Run the Python migrate script and capture its exit status
if ! python migrate_postgresql_db.py; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] ERROR: Failed to migrate PostgreSQL database"
    exit 1
fi
