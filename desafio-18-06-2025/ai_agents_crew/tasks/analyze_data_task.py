...
class AnalyzeDataTask:
    def create(
        self, user_query: str, available_dataframes_str: str, agent: Agent
    ) -> Task:
        return Task(
            description=f"""
                Analyze the available dataframes ({available_dataframes_str}) to answer the following user's query: '{user_query}'.
